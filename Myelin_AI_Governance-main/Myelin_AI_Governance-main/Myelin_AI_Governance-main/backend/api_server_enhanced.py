...
async def health_check():
    ...
    db = get_db()
    database_connected = await db.verify_access()
    
    return {
        "status": "healthy",
        "orchestrator": "initialized" if orchestrator else "not initialized",
        "database": "connected" if database_connected else "disconnected",
        "database_error": None if database_connected else db.get_last_error()
    }
...
if __name__ == "__main__":
    ...
    print(f"   - Bot detection and anomaly monitoring enabled\n")
