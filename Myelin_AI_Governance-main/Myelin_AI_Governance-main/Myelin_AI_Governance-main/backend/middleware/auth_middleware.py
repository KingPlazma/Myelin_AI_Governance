...
Validates API keys and JWT bearer tokens for protected endpoints.
"""
...
async def validate_api_key(request: Request) -> Dict[str, Any]:
    ...
    if api_key.startswith(settings.API_KEY_PREFIX):
        ...
        claims = jwt_service.verify_access_token(api_key)
        db = get_db()
        ...
        organization = await db.get_organization(user["organization_id"])
        auth_context = {
            "api_key": None,
            "token_type": "jwt",
            "claims": claims,
