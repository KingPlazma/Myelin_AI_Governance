...
async def refresh_jwt(payload: TokenRefreshRequest, request: Request, response: Response):
    ...
    return TokenResponse(**jwt_service.rotate_tokens(payload.refresh_token, user))
...
async def update_api_key(
    ...
    updated_key = await db.update_api_key(key_id, organization["id"], update_dict)
    if not updated_key:
        raise HTTPException(
    ...
    return APIKeyResponse(
        ...
        rate_limit_per_minute=updated_key["rate_limit_per_minute"]
    )
