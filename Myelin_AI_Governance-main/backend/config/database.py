...
class Database:
    ...
    def _initialize_client(self):
        ...
        try:
            ...
            # Use explicit credentials and named database support for Firestore.
            sa_credentials = google_service_account.Credentials.from_service_account_file(
                settings.FIREBASE_CREDENTIALS_JSON
            )
            database = None if settings.FIREBASE_DATABASE_ID in (None, "", "(default)") else settings.FIREBASE_DATABASE_ID
    ...
    async def verify_access(self) -> bool:
        """Verify that the Firebase client can make an actual Firestore request."""
        if not self.client:
            return False

        try:
            next(self.client.collection("organizations").limit(1).stream(), None)
            self.last_error = None
            return True
        except Exception as e:
            self.last_error = str(e)
            logger.error(f"Error verifying Firebase access: {e}")
            return False

    def get_last_error(self) -> Optional[str]:
        """Return the most recent database error, if any."""
    ...
    async def update_api_key(self, key_id: str, organization_id: str, update_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Update editable API key fields for an organization-owned key."""
        if not self.client:
            return None
