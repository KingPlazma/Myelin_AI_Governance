...
DEMO_BACKEND_URL = os.getenv("DEMO_BACKEND_URL", "http://127.0.0.1:8011")
...
def create_demo_zip() -> bytes:
    ...
    with zipfile.ZipFile(buffer, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        for filename in ["backend.py", "frontend.py", "requirements.txt", "README.md", "test_demo.py"]:
            file_path = DEMO_DIR / filename
            if file_path.exists():
                zf.write(file_path, arcname=f"chatbot_demo/{filename}")
    buffer.seek(0)
    return buffer.read()
...
# Download package
st.markdown("### Download the demo package")
zip_data = create_demo_zip()
st.download_button(
    label="Download chatbot agent package",
    data=zip_data,
    file_name="chatbot_demo.zip",
