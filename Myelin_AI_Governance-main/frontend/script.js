...
function resolveBackendOrigin() {
  const storedOrigin = localStorage.getItem('myelin_backend_origin');
  if (storedOrigin) {
    ...
    showModal(
      ...
      `
        ...
        <p style="margin-top: 15px; font-size: 13px;">
          ...
          1. Make sure the backend is running at ${BACKEND_ORIGIN}<br>
          2. Check the browser console (F12) for details<br>
          3. Verify Firebase Firestore is connected
        </p>
      `
    );
  }
...
async function testBackendConnection() {
  try {
    ...
    showNotification(`Cannot connect to backend at ${BACKEND_ORIGIN}.`, 'error');
...
function viewDocumentation() {
  window.open(`${BACKEND_ORIGIN}/docs`, '_blank');
}
