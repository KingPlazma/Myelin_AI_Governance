...
function getApiKeyUiElements() {
  return {
    panel: document.getElementById('api-key-inline'),
    field: document.getElementById('generated-api-key-field'),
    toggleButton: document.getElementById('toggle-api-key-visibility'),
    copyButton: document.getElementById('copy-api-key')
  };
}

function getEyeIconSvg(isVisible) {
  if (isVisible) {
    return '<svg viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path d="M2.1 3.5 1 4.6 5 8.6C3.2 9.5 1.8 10.9 1 12c1.2 3.6 5.5 7 11 7 2.3 0 4.3-.6 6-1.5l3.4 3.4 1.1-1.1L2.1 3.5zM12 16a4 4 0 0 1-3.7-2.5l1.2 1.2a2.4 2.4 0 0 0 3.4 0l1.5 1.5A4 4 0 0 1 12 16zm0-8c2.2 0 4 1.8 4 4 0 .6-.1 1.2-.4 1.7L10.3 8.4c.5-.3 1.1-.4 1.7-.4zm0-3c5.5 0 9.8 3.4 11 7-.5 1.4-1.5 2.8-2.9 4l-1.2-1.2c.9-.8 1.6-1.7 2.1-2.8-1.4-2.9-5-5-9-5-.8 0-1.5.1-2.2.3L8.1 5.6C9.3 5.2 10.6 5 12 5z"></path></svg>';
  }

  return '<svg viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path d="M12 5C6.5 5 2.2 8.4 1 12c1.2 3.6 5.5 7 11 7s9.8-3.4 11-7c-1.2-3.6-5.5-7-11-7zm0 11a4 4 0 1 1 0-8 4 4 0 0 1 0 8z"></path></svg>';
}

function renderApiKeyField() {
  const { panel, field, toggleButton, copyButton } = getApiKeyUiElements();
  if (!panel || !field || !toggleButton || !copyButton) {
    return;
  }
  ...
  if (!currentApiKey) {
    ...
    toggleButton.setAttribute('aria-label', 'Show key');
    toggleButton.innerHTML = getEyeIconSvg(false);
    return;
  }

  panel.hidden = false;
  field.value = isApiKeyVisible ? currentApiKey : maskApiKey(currentApiKey);
  toggleButton.disabled = false;
  copyButton.disabled = false;
  toggleButton.title = isApiKeyVisible ? 'Hide key' : 'Show key';
  toggleButton.setAttribute('aria-label', isApiKeyVisible ? 'Hide key' : 'Show key');
  toggleButton.innerHTML = getEyeIconSvg(isApiKeyVisible);
}

function clearApiKeyForNewGeneration() {
  currentApiKey = null;
...
function setApiGenerationState(isLoading) {
  const trigger = document.querySelector('[data-backend-action="generate-demo-key"]');
  if (!trigger) {
    return;
  }
...
async function copyApiKey() {
  if (!currentApiKey) {
    showToast('No API key available to copy yet.', 'error');
    return;
  }
  ...
  try {
    ...
    showToast('Could not copy key. Please copy it manually.', 'error');
  }
}
...
async function createDemoApiKey() {
  ...
  setApiGenerationState(true);
  clearApiKeyForNewGeneration();

  try {
    let data;

    try {
      data = await fetchJson(`${BACKEND_BASE_URL}/api/v1/public/demo-api-key`, {
        method: 'POST',
        body: JSON.stringify(payload)
      });
    } catch (error) {
      const message = String(error?.message || '');
      if (!message.toLowerCase().includes('organization name already taken')) {
        throw error;
      }

      // Rare fallback: retry once with a unique org name if backend reports a collision.
      data = await fetchJson(`${BACKEND_BASE_URL}/api/v1/public/demo-api-key`, {
        method: 'POST',
        body: JSON.stringify({
          ...payload,
          organization_name: `Myelin Demo Org ${Date.now()}`
        })
      });
    }
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const toggleApiKeyButton = document.getElementById('toggle-api-key-visibility');
