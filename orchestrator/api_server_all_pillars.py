...
async def audit_conversation(request: ConversationRequest):
    ...
    if app.state.factual and request.source_text:
        try:
            score, report = app.state.factual.evaluate(request.bot_response, request.source_text)
            results["pillars"]["factual"] = {
                "status": "success",
                "final_score": score,
                "report": report
            }
        except Exception as e:
            results["pillars"]["factual"] = {"status": "error", "error": str(e)}
...
class FactualRequest(BaseModel):
    model_output: str = Field(..., description="The text generated by the model")
    source_text: Optional[str] = Field(None, description="Source/Reference text for factual verification")

@app.post("/api/v1/audit/factual")
async def audit_factual(request: FactualRequest):
    """Factual consistency audit"""
    if not app.state.factual:
        # If running in mock/lite mode, might fail or return mock
        raise HTTPException(status_code=503, detail="Factual pillar not loaded or in error state")
        
    try:
        # Factual pillar evaluate method signature: evaluate(model_output, source_text)
        # Note: In some versions it might vary, but based on load_all_pillars it's FactualEnsembleManager
        score, report = app.state.factual.evaluate(request.model_output, request.source_text)
        return {
            "status": "success", 
            "final_score": score,
            "report": report
